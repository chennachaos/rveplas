...
CDOC C                          accepts NTYPE=1 (plane stress)
CDOC C                          NTYPE=2 (plane strain) and
CDOC C                          NTYPE=3 (axisymmetric).
...
CDOC DOUBLE_PRECISION THICK  <> Thickness of the current point. Initial
CDOC C                          (reference) value on entry, current
...
CHST
CHST E.de Souza Neto, July   2003: Thickness update changed. Now uses
CHST                               reference (and not previous)
CHST                               thickness on entry.
CHST
      SUBROUTINE SUOGD
