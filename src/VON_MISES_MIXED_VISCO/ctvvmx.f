CDOC BEGIN_SUBROUTINE CTVVMX
CDOC Consistent tangent for visco-plastic von Mises model/mixed hard.
CDOC
CDOC This routine computes the tangent matrix consistent with the
CDOC fully implicit elastic predictor/visco-plastic corrector algorithm
CDOC for the visco-plastic von Mises model with mixed hardening and
CDOC Peric's power law for plastic flow. The corresponding integration
CDOC algorithm is coded in subroutine SUVVMX.
...
CDOC C                          with the algorithm
CDOC C                          implemented in routine \smparm{SUVVMX}.
CDOC INTEGER          IPROPS >  Array of integer material properties.
CDOC C                          This array is set in routines
CDOC C                          \smparm{MATIRD} and \smparm{RDVVMX}.
...
CHST                  January 2012: Error message added for
CHST                                incorporation into HYPLAS
CHST
      SUBROUTINE CTVVMX
     1(   DGAMA      ,DTIME      ,DMATX      ,EPFLAG     ,IPROPS     ,
...
C VISCO-PLASTIC MODEL WITH PIECE-WISE LINEAR MIXED ISOTROPIC/KINEMATIC
C HARDENING AND PERIC's POWER LAW FOR VISCO-PLASTIC FLOW.
C PLANE STRAIN AND AXISYMMETRIC IMPLEMENTATIONS ONLY.
C
C REFERENCE: Section 11.6.3 (note that the model used in this routine
C            is a generalisation of the isotropically hardening model
C            referred to in Section 11.6.3 to include mixed
C            isotropic/kinematic hardening).
C***********************************************************************
C Stops program if neither plane strain nor axisymmetric state
      IF(NTYPE.NE.2.AND.NTYPE.NE.3)CALL ERRPRT('EI0062')
...
C Assemble elastoplastic tangent (upper triangle only)
        ...
        AUX=(QBAR*RTSEN)/(DGAMA+DTIME*R1DVIS)
        BFACT=R6*GMODU*GMODU*(DGAMA/QBARTR-
     1        R1/(AUX+R3G+QBAR/SIGMAY*HISLOP+HKSLOP))/(ETANOR*ETANOR)
        DO 50 I=1,NSTRE
          DO 40 J=I,NSTRE
