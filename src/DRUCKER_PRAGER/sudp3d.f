...
CHST
     ...
     2    STRAT(MSTRE)       ,STRES(MSTRE)       ,STREP(MSTRE)
      DIMENSION
     1    STRIAL(MSTRE)
      DATA
...
C Stops program if neither plane strain nor axisymmetric
      IF(NTYPE.NE.4)CALL ERRPRT('EI0016')
...
C -----------------------------------------------------------
   50   CONTINUE
        STRES(1)=FACTOR*STRIAL(1)
        STRES(2)=FACTOR*STRIAL(2)
        STRES(4)=FACTOR*STRIAL(4)
        STRES(5)=FACTOR*STRIAL(5)
        STRES(6)=FACTOR*STRIAL(6)
        STRES(3)=FACTOR*STRIAL(3)
ccccccccccccccccccccccccccccccccccccccccc      
c        C PLASTIC STEP OF EP     
!         IF(FACTOR.EQ.0)THEN
!         VARJ3=RP5*(P*P+P*P+P*P)
!         ELSE
        
      VARJ3=STRES(4)*STRES(4)+STRES(5)*STRES(5)+
     1       STRES(6)*STRES(6)+ RP5*(STRES(1)*STRES(1)+
     2       STRES(2)*STRES(2)+STRES(3)*STRES(3))
C      
        JJ2=SQRT(VARJ3)
            IF(JJ2.EQ.0.0)THEN
            STREP(1)=DGAMA*(ETABAR/3)
            STREP(2)=DGAMA*(ETABAR/3)
            STREP(3)=DGAMA*(ETABAR/3)
            STREP(4)=R0
            STREP(5)=R0
            STREP(6)=R0
            ELSE
        STREP(1)=DGAMA*((1/JJ2)*STRES(1)+ETABAR/3)
        STREP(2)=DGAMA*((1/JJ2)*STRES(2)+ETABAR/3)
        STREP(4)=DGAMA*((1/JJ2)*STRES(4))
        STREP(5)=DGAMA*((1/JJ2)*STRES(5))
        STREP(6)=DGAMA*((1/JJ2)*STRES(6))
        STREP(3)=DGAMA*((1/JJ2)*STRES(3)+ETABAR/3)
           ENDIF
...
C compute converged elastic (engineering) strain components
        ...
        RSTAVA(4)=FACTOR*STRIAL(4)*R2
        RSTAVA(5)=FACTOR*STRIAL(5)*R2
        RSTAVA(6)=FACTOR*STRIAL(6)*R2
        RSTAVA(3)=FACTOR*STRIAL(3)+EEVD3
! C PLASTIC STEP OF EP     
!       VARJ3=STRES(4)*STRES(4)+STRES(5)*STRES(5)+
!      1       STRES(6)*STRES(6)+ RP5*(STRES(1)*STRES(1)+
!      2       STRES(2)*STRES(2)+STRES(3)*STRES(3))
! C      
...
C elastic engineering strain
        ...
        RSTAVA(5)=STRAT(5)
        RSTAVA(6)=STRAT(6)
CSTREP PLASTIC STRAIN COMPONENTS ======================        
        STREP(1)=R0
        STREP(2)=R0
        STREP(4)=R0
        STREP(5)=R0
